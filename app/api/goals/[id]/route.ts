...
import { softDeleteSavingsGoal, updateSavingsGoal } from "@/lib/repositories/goals";
import { createSupabaseServerClient } from "@/lib/supabase/server";
import { firstZodError } from "@/lib/utils/form";
import { savingsGoalUpdateSchema, uuidSchema } from "@/lib/validators/finance.schema";
...
export async function PATCH(request: Request, context: GoalRouteContext) {
  ...
  const parsed = savingsGoalUpdateSchema.safeParse({ ...(body as Record<string, unknown>), id: idParsed.data });
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const { data, error } = await updateSavingsGoal(supabase, user.id, parsed.data);
...
export async function DELETE(_request: Request, context: GoalRouteContext) {
  ...
  const { error } = await softDeleteSavingsGoal(supabase, user.id, idParsed.data);
