...
import { softDeleteSubscription, updateSubscription } from "@/lib/repositories/subscriptions";
import { createSupabaseServerClient } from "@/lib/supabase/server";
import { firstZodError } from "@/lib/utils/form";
import { subscriptionUpdateSchema, uuidSchema } from "@/lib/validators/finance.schema";
...
export async function PATCH(request: Request, context: SubscriptionRouteContext) {
  ...
  const parsed = subscriptionUpdateSchema.safeParse({ ...(body as Record<string, unknown>), id: idParsed.data });
  ...
  const { data, error } = await updateSubscription(supabase, user.id, parsed.data);
  if (error) return NextResponse.json({ success: false, error: { code: "INTERNAL_ERROR", message: error.message } }, { status: 500 });
  return NextResponse.json({ success: true, data });
}

export async function DELETE(_request: Request, context: SubscriptionRouteContext) {
  const { id } = await context.params;
  const idParsed = uuidSchema.safeParse(id);
  if (!idParsed.success) return NextResponse.json({ success: false, error: { code: "VALIDATION_ERROR", message: firstZodError(idParsed.error) } }, { status: 400 });
  ...
  const { error } = await softDeleteSubscription(supabase, user.id, idParsed.data);
  if (error) return NextResponse.json({ success: false, error: { code: "INTERNAL_ERROR", message: error.message } }, { status: 500 });
