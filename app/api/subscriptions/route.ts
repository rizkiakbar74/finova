...
import { createSubscription, listSubscriptions } from "@/lib/repositories/subscriptions";
import { createSupabaseServerClient } from "@/lib/supabase/server";
import { firstZodError } from "@/lib/utils/form";
import { subscriptionCreateSchema } from "@/lib/validators/finance.schema";
...
export async function GET() {
  ...
  const { data, error } = await listSubscriptions(supabase, user.id);
...
export async function POST(request: Request) {
  ...
  const { data, error } = await createSubscription(supabase, user.id, parsed.data);
  if (error) return NextResponse.json({ success: false, error: { code: "INTERNAL_ERROR", message: error.message } }, { status: 500 });
