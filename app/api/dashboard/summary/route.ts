...
import { getDashboardData, defaultDashboardRange } from "@/lib/services/dashboard";
...
export async function GET(request: Request) {
  ...
  if (error || !data) {
    return NextResponse.json(
      { success: false, error: { code: "INTERNAL_ERROR", message: error || "Ringkasan belum bisa dimuat." } },
      { status: 500 }
    );
  }

  return NextResponse.json({ success: true, data: data.summary, meta: { range: data.range } });
}
