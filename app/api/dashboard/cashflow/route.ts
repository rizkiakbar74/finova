...
export async function GET(request: Request) {
  ...
  if (error || !data) {
    return NextResponse.json(
      { success: false, error: { code: "INTERNAL_ERROR", message: error || "Arus kas belum bisa dimuat." } },
      { status: 500 }
    );
  }

  return NextResponse.json({ success: true, data: data.cashflow, meta: { range: data.range } });
}
