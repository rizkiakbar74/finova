...
import { createRecurringBill, listRecurringBills } from "@/lib/repositories/recurring-bills";
import { createSupabaseServerClient } from "@/lib/supabase/server";
import { firstZodError } from "@/lib/utils/form";
import { recurringBillCreateSchema } from "@/lib/validators/finance.schema";
...
export async function GET() {
  ...
  const { data, error } = await listRecurringBills(supabase, user.id);
...
export async function POST(request: Request) {
  ...
  const parsed = recurringBillCreateSchema.safeParse(body);
  ...
  const { data, error } = await createRecurringBill(supabase, user.id, parsed.data);
  if (error) return NextResponse.json({ success: false, error: { code: "INTERNAL_ERROR", message: error.message } }, { status: 500 });
