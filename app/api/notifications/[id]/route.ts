import { NextResponse } from "next/server";
import { archiveNotification, markNotificationRead } from "@/lib/services/notifications";
import { createSupabaseServerClient } from "@/lib/supabase/server";
import { firstZodError } from "@/lib/utils/form";
import { uuidSchema } from "@/lib/validators/finance.schema";

interface NotificationRouteContext {
  params: Promise<{ id: string }>;
}

async function getAuthenticatedContext() {
  const supabase = await createSupabaseServerClient();
  const {
    data: { user },
    error
  } = await supabase.auth.getUser();
  if (error || !user) return { supabase, user: null };
  return { supabase, user };
}

export async function PATCH(_request: Request, context: NotificationRouteContext) {
  const { id } = await context.params;
  const parsed = uuidSchema.safeParse(id);
  if (!parsed.success) return NextResponse.json({ success: false, error: { code: "VALIDATION_ERROR", message: firstZodError(parsed.error) } }, { status: 400 });

  const { supabase, user } = await getAuthenticatedContext();
  if (!user) return NextResponse.json({ success: false, error: { code: "UNAUTHORIZED", message: "You must be logged in." } }, { status: 401 });

  const { data, error } = await markNotificationRead(supabase, user.id, parsed.data);
  if (error) return NextResponse.json({ success: false, error: { code: "INTERNAL_ERROR", message: error.message } }, { status: 500 });
  return NextResponse.json({ success: true, data });
}

export async function DELETE(_request: Request, context: NotificationRouteContext) {
  const { id } = await context.params;
  const parsed = uuidSchema.safeParse(id);
  if (!parsed.success) return NextResponse.json({ success: false, error: { code: "VALIDATION_ERROR", message: firstZodError(parsed.error) } }, { status: 400 });

  const { supabase, user } = await getAuthenticatedContext();
  if (!user) return NextResponse.json({ success: false, error: { code: "UNAUTHORIZED", message: "You must be logged in." } }, { status: 401 });

  const { data, error } = await archiveNotification(supabase, user.id, parsed.data);
  if (error) return NextResponse.json({ success: false, error: { code: "INTERNAL_ERROR", message: error.message } }, { status: 500 });
  return NextResponse.json({ success: true, data });
}
