...
import { createBudgetItem } from "@/lib/repositories/budgets";
import { createSupabaseServerClient } from "@/lib/supabase/server";
import { firstZodError } from "@/lib/utils/form";
import { budgetItemCreateSchema } from "@/lib/validators/finance.schema";
...
export async function POST(request: Request) {
  ...
  const { data, error } = await createBudgetItem(supabase, user.id, parsed.data);
