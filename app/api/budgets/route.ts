...
import { createBudget, listBudgets } from "@/lib/repositories/budgets";
import { createSupabaseServerClient } from "@/lib/supabase/server";
import { firstZodError } from "@/lib/utils/form";
import { budgetCreateSchema } from "@/lib/validators/finance.schema";
...
export async function GET() {
  ...
  const { data, error } = await listBudgets(supabase, user.id);
...
export async function POST(request: Request) {
  ...
  const { data, error } = await createBudget(supabase, user.id, parsed.data);
