...
import { softDeleteBudget, updateBudget } from "@/lib/repositories/budgets";
import { createSupabaseServerClient } from "@/lib/supabase/server";
import { firstZodError } from "@/lib/utils/form";
import { budgetUpdateSchema, uuidSchema } from "@/lib/validators/finance.schema";
...
export async function PATCH(request: Request, context: BudgetRouteContext) {
  ...
  const parsed = budgetUpdateSchema.safeParse({ ...(body as Record<string, unknown>), id: idParsed.data });
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const { data, error } = await updateBudget(supabase, user.id, parsed.data);
...
export async function DELETE(_request: Request, context: BudgetRouteContext) {
  ...
  const { error } = await softDeleteBudget(supabase, user.id, idParsed.data);
